...
function setValue(e) {
    ...
    if (formElement.dataset.field === 'state') {
        const state = document.querySelector('#state') //const state = document.querySelector('#PROP_ST') for new form zip question
        if (!state.value) {
...
// disable for slider function
function isDisabled() {
    const estVal = form.querySelector('#EST_VALText');
    const downPmt = form.querySelector('#DOWN_PMTText');
    const incomeAmt = form.querySelector('#income_amountText');
    const continueBtn = form.querySelector('#continue_btn');

    if (estVal.innerText === "$0" && downPmt.innerText === "$0" && incomeAmt.innerText === "$0") {
        return true
    }
